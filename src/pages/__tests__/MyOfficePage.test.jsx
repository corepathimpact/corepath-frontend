...
test("renders My Office route and shows children section content", async () => {
  renderAuthedAppAt("/my-office");

  await waitFor(() => {
    expect(screen.getByText("Children’s Corner")).toBeInTheDocument();
    expect(screen.getByText("Daniel")).toBeInTheDocument();
  });
});

test("locks Withdraw when eligibility.can_withdraw is false", async () => {
  server.use(
      ...
      res(
        ...
        ctx.json({
          ...
          performance: {
            apat_today: 55,
            apat_average: 72,
            consistency_band: "Needs Attention",
          },
          financials: {
  ...
  await waitFor(() => {
    expect(
      screen.getByText(/Wallet locked due to learning inconsistency/i)
    ).toBeInTheDocument();
