...
export default function SpiritualContent() {
  ...
  const onAdd = async () => {
    try {
      ...
      setError(
        e?.response?.data?.message || e.message || "Failed to add bible verse."
      );
    }
  };
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-slate-600">
        Bible Verses (AREA_06). No day binding. APAT selects per day; RAE
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div className="text-sm text-slate-700 flex items-center">
          Verse Count:{" "}
          <span className="ml-2 font-extrabold">{rows.length}</span> / 30{" "}
          <span
            className={
              rows.length >= 30
                ? "ml-2 text-emerald-700 font-bold"
                : "ml-2 text-amber-700 font-bold"
            }
          >
            {rows.length >= 30 ? "✓" : "⚠"}
          </span>
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <input
            ...
            className="rounded-xl border border-slate-200 px-3 py-2 text-sm"
            placeholder="Translation (e.g., NIV)"
          />
          <textarea
            value={form.text}
            onChange={(e) => setForm((f) => ({ ...f, text: e.target.value }))}
            className="md:col-span-2 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm min-h-[90px]"
            placeholder="Scripture text (immutable; versioned on edit)"
          />
        </div>
        <div className="mt-3">
          <button
