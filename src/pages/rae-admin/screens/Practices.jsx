...
export default function Practices() {
  ...
  const [newPractice, setNewPractice] = useState({ practiceText: "", difficultyLevel: 1 });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!mounted) return;
        setError(e?.response?.data?.message || e.message || "Failed to load practices.");
      } finally {
  ...
  const onAdd = async () => {
    try {
      ...
      setError(e?.response?.data?.message || e.message || "Failed to add practice.");
    }
  };

  return (
    <div>
      <h2 className="text-xl font-extrabold text-slate-900">Practices Management (AREA_05)</h2>
      <p className="mt-1 text-sm text-slate-600">
        Fixed practice set per value (no day binding). Add disabled at 10. No delete (archive via governance).
      </p>
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <input type="number" value={ageMin} onChange={(e) => setAgeMin(e.target.value)} className="rounded-xl border border-slate-200 px-3 py-2 text-sm" />
        <input type="number" value={ageMax} onChange={(e) => setAgeMax(e.target.value)} className="rounded-xl border border-slate-200 px-3 py-2 text-sm" />
      </div>

      <div className="mt-4 flex items-center justify-between">
        <div className="text-sm text-slate-700">
          Practice Count: <span className="font-extrabold">{count}</span> / 10{" "}
          <span className={count >= 6 && count <= 10 ? "text-emerald-700 font-bold" : "text-amber-700 font-bold"}>
            {count >= 6 && count <= 10 ? "✓ Compliant (6–10)" : "⚠ Not compliant"}
          </span>
        </div>
      </div>
      ...
      <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-5">
        <div className="flex items-center justify-between">
          ...
          <div className="text-xs text-slate-500">Draft-only. Approval required to allocate.</div>
        </div>
        <div className="mt-3 grid grid-cols-1 md:grid-cols-4 gap-3">
          <textarea
            value={newPractice.practiceText}
            onChange={(e) => setNewPractice((p) => ({ ...p, practiceText: e.target.value }))}
          ...
          <select
            ...
            onChange={(e) => setNewPractice((p) => ({ ...p, difficultyLevel: e.target.value }))}
            className="rounded-xl border border-slate-200 px-3 py-2 text-sm"
          >
        ...
        <div className="mt-3">
          <button
            onClick={onAdd}
            disabled={!canDraft(role) || !canAddMore || !newPractice.practiceText.trim()}
