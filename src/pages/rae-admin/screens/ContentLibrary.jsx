  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-slate-600">
        Canonical inventory of governed content. View-only (no inline edits, no deletions).
      </p>

      <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
        <input
          value={filters.valueCode}
          onChange={(e) => setFilters((f) => ({ ...f, valueCode: e.target.value }))}
        ...
        <input
          ...
          onChange={(e) => setFilters((f) => ({ ...f, logicArea: e.target.value }))}
      ...
      {loading ? (
        ...
        <div className="mt-6 rounded-xl border border-red-200 bg-red-50 text-red-800 p-4 text-sm">{error}</div>
      ) : (
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-t border-slate-200">
                  <td className="px-4 py-3 font-semibold">{r.valueCode || "—"}</td>
